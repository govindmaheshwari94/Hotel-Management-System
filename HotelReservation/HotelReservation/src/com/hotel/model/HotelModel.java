...
public class HotelModel{
	...
	public static boolean checkAvailability(int roomId, int newBooking) throws IOException{
		Connection conn = null; 
		PreparedStatement stmt = null;
		ResultSet rs = null;
		String selectSQL = "SELECT (h.room_count- coalesce(sum(b.room_count * DATEDIFF(b.check_out, b.check_in)), 0)) as available FROM `booking_info` b, hotel_price h WHERE b.id_price = "+roomId+" and b.id_price = h.id";
	...
	public static ArrayList<HotelLib> getHotel(String search, double rating, double priceStart, double priceEnd, int hotelId) throws IOException, ClassNotFoundException{
		...
		try{
			...
			while(rs.next()){
				...
				PreparedStatement stmt3 = conn.prepareStatement("select id, price, name, room_count from hotel_price where id_hotel = "+tempId);
				ResultSet rs3 = stmt3.executeQuery();

				while(rs3.next()){
					oneHotel.addPrice(rs3.getInt(1), rs3.getDouble(2), rs3.getString(3), rs3.getInt(4));
				}
	...
	public static int addHotelPrice(String id, String price, String name, String room) throws IOException{
		Connection conn = null; 
		PreparedStatement stmt = null;
		int autoId = -1;

		String insertQuery = "INSERT INTO `hotel_price`(`id_hotel`, `price`, `name`, `room_count`) VALUES (?, ?, ?, ?)";
